...
interface IRingBuffer {
     ...
     * be thrown.
     *
     * @param throttled If true, producers will be unable to write when the ring buffer is full. This avoids data loss
     * on the client side, but will block the producer until the consumer has read enough data to free up space in the
     * ring buffer. If false, producers will be able to write even when the ring buffer is full, which will result in
     * data loss on the client side, but will not block the producer. The default value is false.
     */
    void setThrottled(in boolean throttled);
