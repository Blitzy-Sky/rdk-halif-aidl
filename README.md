...
└── <module>/                 # HAL component directory (boot, videodecoder, ...)
    ...
    │   ├── com/rdk/hal/      # AIDL source files (source of truth)
    │   ├── include/ src/     # Generated C++ — gitignored, regenerated by the toolchain
    │   └── docs/             # Component documentation
    └── <version>/            # A released snapshot (e.g. 0.1.0.0) — same layout,
        │                     #   with generated C++ committed (production builds from here)
...
# 4. Commit — only the AIDL source; generated current/ C++ is gitignored
git add bootreason/current/
git commit -m "Update boot interface"
```

Generated C++ for `current/` is written into `bootreason/current/{include,src}/`,
gitignored and regenerated by the toolchain on every build. It is committed only in
a released `<version>/` snapshot (via `./release.sh`), so a **production build from a
released version** needs no Python or AIDL toolchain.
